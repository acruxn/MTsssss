...
const FUELS = ["RON95", "RON97", "Diesel"] as const;
const PRESETS = [20, 50, 100, 200] as const;

export default function FuelPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  const [balance, setBalance] = useState(1234.56);
  const [fuel, setFuel] = useState<string>("RON95");
  ...
  useEffect(() => {
    ...
    if (p.get("prefill") === "1") {
      if (p.get("fuel_type")) setFuel(p.get("fuel_type")!);
  ...
  async function handlePay() {
    setPhase("loading");
    try {
      const r = await postPayment("fuel", amt, { fuel_type: fuel, station: station.trim() || "Nearest station" });
      if (r.success) { setBalance(r.balance); setPhase("success"); }
      else { setError(r.message || "Payment failed"); setPhase("error"); }
  ...
  const Header = () => (
    <div style={{ background: "#0066FF" }} className="px-4 py-3 flex items-center gap-3">
      ...
      <span className="text-white text-lg font-bold">Fuel Payment</span>
    </div>
  );

  if (phase === "loading") return (<div className="min-h-full bg-[#F5F5F5]"><Header /><div className="flex flex-col items-center py-32"><div className="w-12 h-12 border-[3px] border-gray-200 border-t-[#0066FF] rounded-full animate-spin"/><p className="text-gray-500 mt-4 text-sm">Processing payment...</p></div></div>);
  ...
  if (phase === "success") return (
    <div className="min-h-full bg-[#F5F5F5]"><Header />
      <div className="flex flex-col items-center text-center px-6 py-16">
        ...
        <p className="text-gray-500 text-sm mb-6">RM {amt.toFixed(2)} for {fuel}</p>
        <div className="bg-white rounded-2xl p-4 w-full mb-6 shadow-sm">
          <div className="flex justify-between py-2 border-b border-gray-50"><span className="text-sm text-gray-500">Fuel Type</span><span className="text-sm font-medium">{fuel}</span></div>
  ...
  if (phase === "error") return (<div className="min-h-full bg-[#F5F5F5]"><Header /><div className="flex flex-col items-center text-center px-6 py-16"><div className="w-16 h-16 rounded-full bg-red-100 flex items-center justify-center mb-4"><svg className="w-8 h-8 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M6 18L18 6M6 6l12 12"/></svg></div><h2 className="text-xl font-bold text-gray-900 mb-1">Payment Failed</h2><p className="text-gray-500 text-sm mb-6">{error}</p><button onClick={() => setPhase("form")} className="w-full py-3 rounded-xl text-white font-semibold" style={{ background: "#0066FF" }}>Try Again</button></div></div>);
    ...
    <div className="min-h-full bg-[#F5F5F5]">
      ...
      <div className="px-4 py-4">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
          ...
          <div>
            <label className="text-xs font-medium text-gray-500 mb-2 block">Fuel Type</label>
            <div className="flex gap-2">
              {FUELS.map(f => (
                <button key={f} onClick={() => setFuel(f)} className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border-2 transition-colors ${fuel === f ? "border-[#0066FF] bg-[#EBF5FF] text-[#0066FF]" : "border-gray-200 text-gray-500"}`}>{f}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-gray-400">RM</span>
              <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="Custom amount" className="w-full border border-gray-200 rounded-xl pl-12 pr-4 py-3 text-sm focus:outline-none focus:border-[#0066FF]" />
            </div>
            {amt > balance && <p className="text-xs text-red-500 mt-1">Insufficient balance</p>}
          </div>
          <div>
            <label className="text-xs font-medium text-gray-500 mb-1 block">Station (optional)</label>
            <input value={station} onChange={e => setStation(e.target.value)} placeholder="e.g. Petronas KLCC" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#0066FF]" />
          </div>
        </div>
        <button onClick={handlePay} disabled={!canSubmit} className="w-full mt-4 py-3.5 rounded-xl text-white font-semibold disabled:opacity-40" style={{ background: "#0066FF" }}>Pay RM {amt > 0 ? amt.toFixed(2) : "0.00"}</button>
      </div>
    </div>
  );
}
