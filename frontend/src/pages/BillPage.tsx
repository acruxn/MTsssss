...
export default function BillPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  async function handlePay() {
    setPhase("loading");
    try {
      const r = await postPayment("bill", amt, { biller: biller.trim(), account_no: accountNo.trim() });
      if (r.success) {
  ...
  const Header = () => (
    <div style={{ background: "#0066FF" }} className="px-4 py-3 flex items-center gap-3">
      ...
      <span className="text-white text-lg font-bold">Pay Bills</span>
    </div>
  );
  ...
  if (phase === "loading") return (
    <div className="min-h-full bg-[#F5F5F5]">
      ...
      <div className="flex flex-col items-center justify-center py-32">
        ...
        <p className="text-gray-500 mt-4 text-sm">Processing payment...</p>
      </div>
    </div>
  );
  ...
  if (phase === "success") return (
    <div className="min-h-full bg-[#F5F5F5]">
      ...
      <div className="flex flex-col items-center text-center px-6 py-16">
        ...
        <p className="text-gray-500 text-sm mb-6">RM {amt.toFixed(2)} paid to {biller}</p>
        <div className="bg-white rounded-2xl p-4 w-full mb-6 shadow-sm">
          <div className="flex justify-between py-2 border-b border-gray-50"><span className="text-sm text-gray-500">Biller</span><span className="text-sm font-medium">{biller}</span></div>
          <div className="flex justify-between py-2 border-b border-gray-50"><span className="text-sm text-gray-500">Account No.</span><span className="text-sm font-medium">{accountNo}</span></div>
      ...
      <div className="px-4 py-4">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
          ...
          <div>
            ...
            <input value={accountNo} onChange={e => setAccountNo(e.target.value)} placeholder="e.g. 220012345678" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#0066FF] focus:ring-1 focus:ring-[#0066FF]/20" />
          </div>
          <div>
        ...
        <button onClick={handlePay} disabled={!canSubmit} className="w-full mt-4 py-3.5 rounded-xl text-white font-semibold transition-colors disabled:opacity-40" style={{ background: "#0066FF" }}>Pay RM {amt > 0 ? amt.toFixed(2) : "0.00"}</button>
      </div>
    </div>
  );
}
